PARAMETERS
    ...
    NCST(NT , *)    NONELECTRIC COST COEFFICIENTS - $ PER GJ
    URACST(URA,*)   ura COST COEFFICIENTS - $ PER GJ
    CYCOMBCST(CYCSTP,*)     CYCLE COST COEFFICIENT - $ PER GJ (NOT EVERYTHING)
    RSV(x,*)        PROVEN RESERVES - EXAJ
    RSVURA(URA,*)   PROVEN RESERVES OF ura - EXAJ
    RSC(x,*)        UNDISCOVERED RESOURCES - EXAJ
    RSCURA(URA,*)   UNDISCOVERED ura RESOURCES - EXAJ
    PRV(x,*)        UPPER BOUNDS ON PRODUCTION-RESERVE RATIOS
    RDF(x,*)        UPPER BOUNDS ON RESOURCE DEPLETION FACTORS
    RDFURA(ura,*)   UPPER BOUNDS ON ura RESOURCE DEPLETION FACTOR
    ...
    Res_marg_share(tp,rg,sw)  Share of reserve margin in total demand
    ...
    sscap(tp,rg,sw)    Share of shoret-term storage from reserve margin in total demand
    SSprod(tp,rg,sw)   Production of shoret-term storage from reserve margin in total demand
