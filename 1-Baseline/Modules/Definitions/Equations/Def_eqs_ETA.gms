supelec(rg,pp,sw)$st(pp,sw)..
                ...
                (1-peakshare(pp,rg,sw))*E(rg,pp,sw) + eeis(pp,rg) * (EISS(pp,rg,sw) - eisdemand(pp,rg))+
                htrth2("electrolysis",pp,rg) * PH("electrolysis",pp,rg,sw) ;
...
elecgfrac(rg,pp,sw)$st(pp,sw)..
                ...
                0.6 *(peakshare(pp,rg,sw)-Res_marg_share(pp,rg,sw)*sscap(pp,rg,sw))*E(rg,pp,sw);
...
coalpeaksh(rg,pp,sw)$st(pp,sw)..
                PE("coal-n",pp,"peak",rg,sw)+PE("coal-a",pp,"peak",rg,sw) =l= pkcoal*(peakshare(pp,rg,sw)-Res_marg_share(pp,rg,sw)*sscap(pp,rg,sw))*E(rg,pp,sw);                 
...
costnrg(rg,tp,sw)$(pp(tp) and st(tp,sw))..    EC(rg,tp,sw) =g= .001 * (              
                    ...
                    + sum(ht, PH(ht,tp,rg,sw)*hcst(ht,tp,rg)* cstred(tp)+H2T(tp, rg))
                    + sum(nt, PN(nt,tp,rg,sw)*ncst(nt,rg)* cstred(tp))


$onText
                    + .5 * sum (et, (qecst(et,tp,rg)  *

   sum(ts, PE(et,ts,tp,rg,sw))  *   sum(ts,PE(et,ts,tp,rg,sw)) / ecap(et,tp,rg)) $
...
dech2(ht,rg,tp10+1,sw)$st(tp10+1,sw)..      
                 ...
                 PH(ht,tp10,rg,sw)*(decf(rg)**nyper(tp10));
exph2(rg,tp10+1,ht,sw)$st(tp10+1,sw)..
             PH(ht,tp10+1,rg,sw) - nshf(rg)*0.1 * N(rg,tp10+1,sw)
       =l=            (nxpf(rg)**nyper(tp10)) * PH(ht,tp10,rg,sw);
expe(rg,tp+1,xle,sw)$st(tp+1,sw)..
             sum(ts,PE(xle,tp+1,ts,rg,sw))  - nshf(rg) * E(rg,tp+1,sw)
