EQUATIONS
    ...
    RNEWConstraint(rg,tp,nt,sw)        MAX Cap on the production with RNEW 
    ...
    expnuc(rg,tp,etNUC,sw)   expansion rate of nuc
    ...
    rdfuralim(rg,tp,ura,sw)    ura resource depletion limit
    prvlim(rg,tp,x,sw)    production-reserve limit
    prvuralim(rg,tp,ura,sw)   production-reserve limit ura
    ...
    fracsumDAC(tp,rg,ps,sw)  sum of fractional DACCS programs 
    ...
    prodMOX(tp,rg,sw)     Production de MOX possible
