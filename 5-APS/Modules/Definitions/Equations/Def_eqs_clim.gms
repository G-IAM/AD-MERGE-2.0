carlev(rg,pp,sw)$st(pp,sw)..
              sum((et,ts), cece(et,pp,rg)*PE(et,pp,ts,rg,sw))
                    ...
                    + cecn("RNEW",pp,rg) *  PN("RNEW",pp,rg,sw)
                    + cecn("neb-hc",pp,rg) *  PN("neb-hc",pp,rg,sw)                  
                    ...
                    - SUM((ps), cabDAC(pp,rg,ps)*DAC1(pp,rg,ps,sw))
                    - SUM((ps), cabDAC(pp,rg,ps)*DAC2(pp,rg,ps,sw))
                    + fos("carbdif",rg)
                    =e=  CLEV(pp,rg,sw) + CRLX(pp,rg,sw);


fracsum(sw)..    SUM((sy,ps) , AFF(sy,ps,sw)) =l=  1;
fracsumDAC(tp,rg,ps,sw)..    DAC1(tp,rg,ps,sw)+DAC2(tp,rg,ps,sw) =l=  1;
