...
PARAMETERS
    ...
    MD_AD(*,tp,*)       Damages and adaptation REPORT 
    S_CC(*,tp,*)         Share of climate change cost shares in total - percent   
    MAC(SW,RG,Tp)       MARGINAL ABATEMENT COST - $ PER TON
    ;
...
CARGDP(tp,"world")      =  1000*SUM(rg, TOTCARB(rg,tp,"sw1"))/
                           SUM(rg,      VALUESUM("sw1",rg,tp,"gdp"));
MD_AD("MD",pp,rg)= MD.L(rg,pp,'sw1');
MD_AD("Resi_MD",pp,rg)=RD.L(rg,pp,'sw1')*POTGDP(pp,rg);
MD_AD("ReA_AD",pp,rg)=FAD.L(rg,pp,'sw1')*POTGDP(pp,rg);
MD_AD("ProA_AD",pp,rg)= IAD.L(rg,pp,'sw1')*POTGDP(pp,rg);
MD_AD("AD",pp,rg)= (IAD.L(rg,pp,'sw1')+FAD.L(rg,pp,'sw1'))*POTGDP(pp,rg);
MD_AD("MD%",pp,rg)= MD.L(rg,pp,'sw1')/POTGDP(pp,rg)*100;
MD_AD("Resi_MD%",pp,rg)=RD.L(rg,pp,'sw1')*100;
...
MD_AD("ProA_AD",pp,"world")=sum(rg,IAD.L(rg,pp,'sw1')*POTGDP(pp,rg));
MD_AD("AD",pp,"world")= sum(rg,(IAD.L(rg,pp,'sw1')+FAD.L(rg,pp,'sw1'))*POTGDP(pp,rg));
MD_AD("MD%",pp,"world")= sum(rg,MD.L(rg,pp,'sw1'))/sum(rg,POTGDP(pp,rg))*100;
MD_AD("Resi_MD%",pp,"world")= sum(rg,RD.L(rg,pp,'sw1')*POTGDP(pp,rg)) /sum(rg,POTGDP(pp,rg))*100 ;
...
SelectedPEL(sw,rg,tp,"Nuclear")=  sum(ts,PE.L("nuc-LWR",tp,ts,rg,sw)+PE.L("nuc-SMR",tp,ts,rg,sw)+PE.L("nuc-VHTR",tp,ts,rg,sw)+PE.L("nuc-SFR",tp,ts,rg,sw))*1000+0.00000001;
...
PEPCT(sw,rg,tp,et)      = PEPCT(sw,rg,tp,et)$ (PEPCT(sw,rg,tp,et) ge .00000001);
PEPCT(sw,"world",tp,et) = PEPCT(sw,"world",tp,et)$ (PEPCT(sw,"world",tp,et) ge .00000001);

PNL(sw,rg,tp,"cldu")       =  PN.L("cldu",  tp,rg,sw) $ (PN.L("cldu",  tp,rg,sw) GE .0000001);
PNL(sw,rg,tp,"synf")       =  PN.L("synf",  tp,rg,sw) $ (PN.L("synf",  tp,rg,sw) GE .0000001);
PNL(sw,rg,tp,"rnew")       =  PN.L("rnew",  tp,rg,sw) $ (PN.L("rnew",  tp,rg,sw) GE .0000001);
PNL(sw,rg,tp,"neb-hc")     =  PN.L("neb-hc",tp,rg,sw) $ (PN.L("neb-hc",tp,rg,sw) GE .0000001);
PNL(sw,rg,tp,"neb-lc")     =  PN.L("neb-lc",tp,rg,sw) $ (PN.L("neb-lc",tp,rg,sw) GE .0000001);
PNL(sw,rg,tp,"lbdn")       =  PN.L("lbdn",  tp,rg,sw) $ (PN.L("lbdn",  tp,rg,sw) GE .0000001);
PNL(sw,rg,tp,"oilnon")     =  OILNON.L(tp,rg,sw)      $ (OILNON.L(tp,rg,sw)      GE .0000001);
PNL(sw,rg,tp,"gasnon")     =  GASNON.L(tp,rg,sw)      $ (GASNON.L(tp,rg,sw)      GE .0000001);
PNL(sw,rg,tp,"H2")         =  PN.L("H2",  tp,rg,sw)   $ (PN.L("H2",  tp,rg,sw) GE .0000001);
...
XTOTCARB(SW,"2020",RG)       = CARLIM("sw1","2020",rg);
...
RMD(sw,tp,rg)             = MD.L(rg,tp,sw);
RND(sw,tp,rg)= MD.L(rg,tp,'sw1')/POTGDP(tp,rg)*100;
RND(sw,tp,"World")=sum(rg,MD.L(rg,tp,'sw1'))/POTGDP(tp,"World")*100;
...
RAvC(sw,tp,rg)=RGD(sw,tp,rg)-RND(sw,tp,rg);
RAvC(sw,tp,"World")=RGD(sw,tp,"World")-RND(sw,tp,"World");
...
PRIMNURE(SW,TP,RG) = sum(ts,PE.L("HYDRO",TP,ts,RG,SW))*HTRT("HYDRO",tp,RG) + sum(ts,PE.L("NUC-LWR",TP,ts,RG,SW))*HTRT("NUC-LWR",tp,RG) +
                     sum(ts,PE.L("nuc-SMR",tp,ts,rg,sw))*HTRT("NUC-SMR",tp,RG)+sum(ts,PE.L("nuc-VHTR",tp,ts,rg,sw))*HTRT("NUC-VHTR",tp,RG)+sum(ts,PE.L("nuc-SFR",tp,ts,rg,sw))*HTRT("NUC-SFR",tp,RG)+
...
PRIMNU(SW,TP,RG) = sum(ts,PE.L("NUC-LWR",TP,ts,RG,SW))*HTRT("NUC-LWR",tp,RG)+
                     sum(ts,PE.L("nuc-SMR",tp,ts,rg,sw))*HTRT("NUC-SMR",tp,RG)+sum(ts,PE.L("nuc-VHTR",tp,ts,rg,sw))*HTRT("NUC-VHTR",tp,RG)+sum(ts,PE.L("nuc-SFR",tp,ts,rg,sw))*HTRT("NUC-SFR",tp,RG);
