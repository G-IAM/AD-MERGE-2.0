...
display_timer() {
  ...
  while ps -p $gams_pid > /dev/null; do
    ...
    # Use printf to display elapsed time and overwrite the same line
    printf "Elapsed time: %d hours, %d minutes, %d seconds\r" $hours $minutes $seconds
    sleep 1
  done
}
...
nohup gams 1-restart_solver.gms restart=restart gdx=default o=output_REF_wA.lst action=CE --input=no_lbd-bline.gms > Terminal-Run-AD-MERGE2.txt 2>&1 &
gams_pid=$!
