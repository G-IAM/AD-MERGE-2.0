...
*PREMIT(sw,pp,ghg)    =  1000*TOTEMIT.M(ghg,pp,sw)/TRDBAL.M(pp,"nmr",sw);

*PREMITO(sw,pp,ghg) $((ord(ghg) ne 1) and PREMIT(sw,pp,"co2"))
*                     =  PREMIT(sw,pp,ghg)/ PREMIT(sw,pp,"co2");
...
PHL(sw,rg,tp,"coal-h2")          =       (PH.L("coal-h2",  tp,rg,sw) $ (PH.L("coal-h2",  tp,rg,sw) GE .1)+0.0000001)*1000;
PHL(sw,rg,tp,"gas-h2")           =        (PH.L("gas-h2",  tp,rg,sw) $ (PH.L("gas-h2",  tp,rg,sw) GE .1)+0.0000001)*1000;
PHL(sw,rg,tp,"gas-h2-CCS")       =    (PH.L("gas-h2-CCS",  tp,rg,sw) $ (PH.L("gas-h2-CCS",  tp,rg,sw) GE .1)+0.0000001)*1000;
PHL(sw,rg,tp,"coal-h2-CCS")      =   (PH.L("coal-h2-CCS",  tp,rg,sw) $ (PH.L("coal-h2-CCS",  tp,rg,sw) GE .1)+0.0000001)*1000;
...
XTOTEM(sw,"2015","CH4")           =    390;
XTOTEM(sw,"2020","CH4")           =    400;
...
MD_AD("RAvC%",pp,rg)= RAvC("sw1",pp,rg);
MD_AD("RAvC%",pp,"world")= RAvC("sw1",pp,"World");
...
PRIMNURE(SW,TP,RG) = sum(ts,PE.L("HYDRO",TP,ts,RG,SW))*HTRT("HYDRO",tp,RG) + sum(ts,PE.L("NUC",TP,ts,RG,SW))*HTRT("NUC",tp,RG) +
...
PRIMNU(SW,TP,RG) = sum(ts,PE.L("NUC",TP,ts,RG,SW))*3.6;
...
DISPLAY E_H1 ,E_H2 , E_H3, E_H4, E_Hp, E_CU, E_sscap,E_ssc,E_peak, E_SSprod,Elec_H2curtail;
